...
if uploaded_file:
    try:
        ...
        preview_cols = ['乗務員'] + ([date_col] if date_col else []) + ['走行距離_km','燃料使用量_L','燃料費_円','アイドリング率_％','平均速度_km_h']
        st.dataframe(df[preview_cols])
        # 集計
        summary = df.groupby('乗務員', as_index=False).agg(
            走行距離_km=('走行距離_km','sum'), 燃料使用量_L=('燃料使用量_L','sum'), 燃料費_円=('燃料費_円','sum'),
            稼働時間_分=('稼働時間_分','sum'), アイドリング時間_分=('アイドリング時間_分','sum'), 走行時間_分=('走行時間_分','sum')
        )
        summary['月間平均燃費_km_L'] = np.where(summary['燃料使用量_L']>0, (summary['走行距離_km']/summary['燃料使用量_L']).round(2), pd.NA)
        summary['月間アイドリング率_％'] = np.where(summary['稼働時間_分']>0,(summary['アイドリング時間_分']/summary['稼働時間_分']*100).round(2),pd.NA)
        # 月間集計表示
        ...
        st.subheader('📊 月間燃料使用量ランキング'); plot_bar(summary,'燃料使用量_L','ドライバー別 月間燃料使用量 (L)')
        st.subheader('📊 月間燃料費ランキング'); plot_bar(summary,'燃料費_円','ドライバー別 月間燃料費 (円)')
        # アイドリング率ランキング
        summary['アイドリング色'] = np.where(summary['月間アイドリング率_％']>=idling_threshold,'red','blue')
        st.subheader('📊 月間アイドリング率ランキング')
        fig2 = px.bar(summary.sort_values('月間アイドリング率_％',ascending=False),x='乗務員',y='月間アイドリング率_％',color='アイドリング色',color_discrete_map={'red':'red','blue':'blue'},title=f'ドライバー別 月間アイドリング率 (%) (閾値:{idling_threshold}%)')
        fig2.add_shape(type='line',x0=-0.5,x1=len(summary)-0.5,y0=idling_threshold,y1=idling_threshold,line=dict(color='black',dash='dash'))
